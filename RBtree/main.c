...
int main(int argc, char *argv[])
{
    ...
    while(cmd != EOF ) {
        ...
        printf("$ ");
        fflush(stdout);
        /* Find the first non_whitespace character */
        while (isspace((cmd = getchar()))) {
            if (cmd == '\n') {
                /* We delete up to end of ling after  the
                 * switch, so put the \n back on. */
                 ungetc(cmd, stdin);
                 break;
            }
        ...
        case 'C':
        case 'c':
            /* If we already had a tree, wo need to free up
             * the memory. */
             if (tree != NULL) {
        ...
        case 'i':
            ...
            if (scanf("%d",&arg) != 1) {
                fprintf(stderr, "Error: must specifu integer key to insert.\n");
            }
            else {
                RBinsert(tree, arg);
            }
